<h1 align="center">🍫 Chocolate Sales Dashboard</h1>

<p align="center">
  An interactive Power BI dashboard analysing chocolate sales performance, customer trends, and product insights.
</p>

<br>

<div align="center">
  <img src="Screenshot 2026-06-29 140855.png" alt="Chocolate Dashboard Preview" width="800">
</div>

<br>

<h2>📊 Project Description</h2>

<p>
This dashboard was created using Power BI to explore chocolate sales data and provide insights into revenue, sales trends, product performance, and customer behaviour.

The project demonstrates data cleaning, data modelling, DAX calculations, and interactive visualisation techniques to transform raw sales data into meaningful business insights.
</p>

<div align="center">
  <img src="Screenshot 2026-06-29 141429.png" alt="Chocolate Dashboard Preview" width="800">
</div>

<div align="center">
  <img src="Screenshot 2026-06-29 141811.png" alt="Chocolate Dashboard Preview" width="800">
</div>

<h2>Dax</h2>

<div align="center">
  <img src="Screenshot 2026-08-12 165300.png" alt="Chocolate Dashboard Preview" width="800">
</div>

<p>
  I have encoruged many Dax commands into this projet. Dax is a useful data anaylysis tools that allows us to perform calaulation on the data and use them as metrics for making further analysis to the data. 

  Used DAX extensively to create calculated metrics and support deeper analysis of sales performance. This included using AVERAGE to calculate the average sales generated by each sales assistant, creating calculated columns to identify whether individual targets were met, and using aggregation functions such as SUM to consolidate key performance metrics. These calculations allowed me to compare performance against targets and provide more meaningful insights within the Power BI dashboard. 
</p>

<h2>🔗 View Dashboard</h2>

<p>
  <a href="https://app.powerbi.com/" target="_blank">
    View Power BI Dashboard
  </a>
</p>

<a href="https://app.powerbi.com/links/jduq1eqUGe?ctid=1e341d11-1ce0-4d09-a2a5-22aeaa3eea2a&pbi_source=linkShare">Download Project</a>

<h2>🛠️ Tools Used</h2>

<ul>
  <li>Power BI</li>
  <li>DAX</li>
  <li>Power Query</li>
  <li>Excel</li>
</ul>
<a href="https://github.com/Ayush884400">Back to Portfolio</a>
